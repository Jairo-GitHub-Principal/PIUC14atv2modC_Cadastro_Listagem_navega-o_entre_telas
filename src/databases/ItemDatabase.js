        ...
        return new Promise((resolve) => { //Uma Promise é um objeto que representa a eventual conclusão ou falha de uma operação assíncrona, uma promise é um objeto retornado para o qual você adiciona callbacks, em vez de passar callbacks para uma função.
            ...
            SQLite.echoTest().then(() => {
                ...
                SQLite.openDatabase(database_name, database_version, database_displayname, database_size).then(DB => {db = DB;
                    ...
                    db.executeSql('SELECT 1 FROM cliente LIMIT 1').then(() => {
                        ...
                        db.transaction((tx) => {
                            ...
                            tx.executeSql('CREATE TABLE IF NOT EXISTS cliente (id INTEGER PRIMARY KEY AUTOINCREMENT, nome varchar(30),cpf varchar(11), dataNascimento varchar(30), dataAssiContrato varchar(30), dataPagaLuguel varchar(30))');
                        }).then(() => {
        ...
        return new Promise((resolve) => {  
            ...
            this.Conectar().then((db) => {
                db.transaction((tx) => {
                    ...
                    tx.executeSql('SELECT * FROM cliente', []).then(([tx, results]) => {
                        console.log("Consulta completa");
                        var len = results.rows.length;
                        for (let i = 0; i < len; i++) {
                            let row = results.rows.item(i);
                            const { id, nome, cpf, dataNascimento,dataAssiContrato,dataPagaLuguel } = row;
                            lista.push({ id, nome, cpf, dataNascimento,dataAssiContrato,dataPagaLuguel });
                        }
                        console.log(lista);
                        resolve(lista);
                    });
        ...
        return new Promise((resolve) => {
            this.Conectar().then((db) => {
                db.transaction((tx) => {
                    ...
                    tx.executeSql('INSERT INTO cliente (nome, cpf, dataNascimento,dataAssiContrato,dataPagaLuguel) VALUES (?, ?, ?, ?, ?)', [item.nome, item.cpf, item.dataNascimento, item.dataAssiContrato,item.dataPagaLuguel]).then(([tx, results]) => {
                        resolve(results);
                    });
                }).then((result) => {
                    this.Desconectar(db);
        ...
        return new Promise((resolve) => {
            ...
            this.Conectar().then((db) => {
                db.transaction((tx) => {
                    ...
                    tx.executeSql('UPDATE cliente SET dataPagaLuguel = ?  WHERE id = ?', [item.dataPagaLuguel, item.id]).then(([tx, results]) => {
                        resolve(results);
                    });
                }).then((result) => {
                    this.Desconectar(db);
     ...
     ListarPorID(id) {   //**** CRUD => READ - aqui a tabela é lida *****/      
        return new Promise((resolve) => {  
            ...
            this.Conectar().then((db) => {
                db.transaction((tx) => {
                    //Query SQL para listar os dados da tabela   
                    tx.executeSql('SELECT * FROM cliente WHERE id = ?', [id]).then(([tx, results]) => {
                        console.log("Consulta completa");
                        var len = results.rows.length;
                        for (let i = 0; i < len; i++) {
                            let row = results.rows.item(i);
        ...
        return new Promise((resolve) => {
            this.Conectar().then((db) => {
                db.transaction((tx) => {
                    ...
                    tx.executeSql('DELETE FROM cliente WHERE Id = ?', [id]).then(([tx, results]) => {
                        console.log(results);
                        resolve(results);
                    });
                }).then((result) => {
